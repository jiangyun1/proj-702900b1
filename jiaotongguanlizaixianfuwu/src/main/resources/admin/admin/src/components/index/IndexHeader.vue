<template>
    ...
    <div class="navbar" :style="{backgroundColor:heads.headBgColor,height:heads.headHeight,boxShadow:heads.headBoxShadow,lineHeight:heads.headHeight}">
        <div class="title-menu" :style="{justifyContent:heads.headTitleStyle=='1'?'flex-start':'center'}">
            ...
            <div class="title-name" :style="{fontSize:heads.headFontSize}" style="color: #fff !important;">{{this.$project.projectName}}</div>
            <!--             <img src="../../../../img/logo.jpg" style="width: 60px;height: 60px;border-radius:60px"> -->
        </div>
        <div class="right-menu">
            <div class="user-info" :style="{fontSize:heads.headUserInfoFontSize}" style="color: #fff !important;">{{this.$storage.get('role')}} {{this.$storage.get('adminName')}}</div>
            <div class="logout" :style="{fontSize:heads.headLogoutFontSize}" @click="onLogout" style="color: #fff !important;">退出登录</div>
        </div>
    </div>
</template>
